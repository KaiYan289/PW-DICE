...
class SMODICE_Solver:
    def __init__(self, real_MDP, MDP, time):
        ...
        self.visualizer = Plotter(int(math.sqrt(self.MDP.n)), self.MDP.st, self.MDP.ed, time, directory="res/SMODICE_CHI/fig")
    ...
    def solve(self, TS_dataset, TA_dataset, args):
        ...
        if self.mode == "full":
            ...
            perfect_rho_E = self.compute_marginal_distribution(self.MDP, self.get_expert_policy()).reshape(self.MDP.n, self.MDP.m).sum(axis=1) # mdp_expert for mismatch
            print("rho_E:", rho_E)
            print("perfc:", perfect_rho_E)
            print("diffr:", rho_E - perfect_rho_E)
            # exit(0)
        elif self.mode == "goal":
            # goal-based
            rho_E[self.MDP.ed] = 1
        ...
        H = (self.MDP.gamma * P - B).T @ D @ (self.MDP.gamma * P - B) # |S| x |S|
        y = -((1 - self.MDP.gamma) * p0 + (self.MDP.gamma * P - B).T @ D @ (I + B @ R)) # |S|
        V_star = np.linalg.pinv(H) @ y 
    
        w_star = B @ R + (self.MDP.gamma * P - B) @ V_star + 1 
        
        #for i in range(self.MDP.n):
        #    print("w_star_primal[", i, "] =", w_star[i * self.MDP.m: (i+1) * self.MDP.m])
        
        m = np.array(w_star >= 0, dtype=np.float) # ensure non-negativity
        w_star = w_star * m 
        
        # Weighted BC
        pi_star = (w_star * d).reshape(self.MDP.n, self.MDP.m)
    
        pi_mle = self.solve_MDP()[0] # !!!
        
        for s in range(self.MDP.n):
            # handle ill-posed state via policy bootstrapping heuristic
            if np.isclose(np.sum(pi_star[s, :]), 0):
                #if np.isclose(np.sum(d.reshape(self.MDP.n, self.MDP.m)[s, :]), 0):
                pi_star[s, :] = pi_b[s, :]
                # else:
                #    pi_star[s, :] = pi_mle[s, :]
        #for i in range(self.MDP.n):
        #    print("rho[",i,"] =", pi_star[i], "w_star[", i, "] =", w_star[i * self.MDP.m: (i+1) * self.MDP.m], "d[", i, "] =", d[i * self.MDP.m: (i+1) * self.MDP.m])
        pi_star /= np.sum(pi_star, axis=1, keepdims=True)
        
        f_divergence = d.dot(0.5 * (w_star ** 2))
        
        self.draw_policy(pi_star)
        
        pi_star_argmax = np.zeros_like(pi_star)
        
        for i in range(self.MDP.n):
            pi_star_argmax[i, pi_star[i].argmax()] = 1
        
        self.draw_policy(pi_star_argmax, "SMODICE_argmax_policy")
...
if __name__ == "__main__":
    ...
    # solver = SMODICE_Solver(MDP, MDP_estimate_exact, runtime)
    ...
    t0 = time.time()
    pi_star, f_div, V_star = solver.solve(TS_dataset, TA_dataset, args)
    t1 = time.time()
    print("V_star:", V_star, "f_div:", f_div)
    solver.pi = pi_star
    
    f = open("res/SMODICE_CHI/"+runtime.replace("/", "-").replace(" ", "_")+"aka"+str(time.time())+".txt", "w")
