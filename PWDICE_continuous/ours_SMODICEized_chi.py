...
class Lagrangian_Solver:
    ...
    def solve(self, env, IDM, TS_dataset, TA_dataset, args, dist_model=None):    
        ...
        if args.env_name in ["navigation", "navigation_long"]: self.the_terminal, self.SAVE_CONST = torch.tensor([[0.6, 0.5, 1]]).to(device).double(), args.save_const# 
        elif args.env_name in ["navigation_1d", "navigation_1d_long"]: self.the_terminal, self.SAVE_CONST = torch.tensor([[0.99, 1]]).to(device).double(), args.save_const
        else: self.the_terminal, self.SAVE_CONST = torch.cat([torch.zeros_like(torch.from_numpy(TA_dataset[0]["state"])).to(device).double(), torch.ones(1).to(device).double()]).view(1, -1), 400 if args.env_name == "lunarlander" else args.save_const #
        ...
        if args.scale_dist_with_occupancy > 1e-10 or args.scale_dist_with_context > 1e-10 or args.scale_dist_with_occupancy_product > 1e-10:
            ...
            disc_hyperparam = {"batch_size": min(512, states_TS.shape[0]), "lr": 3e-4, "N": 40000, "lipschitz": args.lipschitz, "suffix": args.env_name + "_" + args.skip_suffix_TA + "_" + args.skip_suffix_TS, "EMA": args.EMA} 
        ...
        train_loader = RepeatedDataset([states_TA.to(device).double(), actions_TA.to(device).double(), terminals_TA.to(device).double(), next_states_TA.to(device).double()], batch_size2)
        ...
        if args.initial == "TS": initials = torch.cat([initials[i].view(1, -1) for i in range(len(initials))], dim=0) # torch.tensor(initials).double().to(device)        
        ...
        for __ in tqdm(range(args.N)):
            ...
            if args.BCdebug != 1:
              for ___ in tqdm(range(len(train_loader))):
                ...
                loss_part1 = epsilon_1 / 2 * ((US1 + US2 - DIST) / epsilon_1 + 1) ** 2 
                ...
                if groundtruth_flag1 == 0:
                    ...
                    if args.train_with_terminal == 1: 
                        loss_part2 = epsilon_2 / 2 * ((-self.gamma * NS0 + IS0 - IS1 + S3 * args.use_s3) / epsilon_2 + 1) ** 2
                    else: 
                        loss_part2 = epsilon_2 / 2 * ((-self.gamma * NS0 * (1 - terminal_I).reshape(-1, 1) + IS0 - IS1 + S3 * args.use_s3) / epsilon_2 + 1) ** 2
    ...
    def Train_WeightedBC(self, s_TA, a_TA, t_TA, ns_TA, initials, args, is_TS=None):
        ...
        if args.BCdebug == 0:
            ...
            for i in tqdm(range((states_TA.shape[0] - 1) // BS + 1)): # first do an empty epoch to find the biggest coefficient
                ...
                assert GT1 == 0, "Error!"          
                if args.EMA > 1e-10:
                    coeff = torch.clamp((-self.gamma * self.ema(next_state, 0) * (1 - terminal).reshape(-1, 1) + self.ema(state, 0) - self.ema(state, 1) + args.use_s3 * self.ema(None, 3)) / epsilon_2, min=0).detach() 
                else: 
                    coeff = torch.clamp((-self.gamma * self.net(next_state, 0) * (1 - terminal).reshape(-1, 1) + self.net(state, 0) - self.net(state, 1) + args.use_s3 * self.net(None, 3)) / epsilon_2, min=0).detach()
...
if __name__ == "__main__":
    ...
    if args.auto == 1: a = "chi-divergence-" + args.env_name + "-" + args.skip_suffix_TA + "-" + args.skip_suffix_TS + '-' + ("threepart" if args.scale_dist_with_context >= 1e-10 else ("twopart" if args.scale_dist_with_occupancy >= 1e-10 else "onepart")) + "-" + args.distance + "-auto"
    else: 
        print("please input description:")
        a = input()
    # a = "hopper-lowlr-bclv2" 
    wandb.init(entity="XXXXX",project="project2-smodiceized", name=str(runtime)+"_"+str(args.seed)+"_ours_smodiceized-deep_"+args.distance+"_"+a)
